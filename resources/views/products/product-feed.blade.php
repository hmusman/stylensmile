        ...
        <table class="table table-striped res-table mar-no" cellspacing="0" width="100%">
            <thead>
                <tr>
                    ...
                    <th>{{translate('Market Price')}}</th>
                    <th>{{translate('Selling Price')}}</th>
                    <th>{{translate('Google Feed')}}</th>
                    <th>{{ translate('Facebook Feed') }}</th>
            ...
            <tbody>
                @foreach($products as $key => $product)
                    <tr>
                        ...
                        <td>{{ number_format($product->unit_price-$product->discount,2) }}</td>
                        <td>
                            <label class="switch">
                                <input onchange="googleFeed(this)"  value="{{ $product->id }}" type="checkbox" <?php if($product->google == 'Yes' && $product->current_stock > 0) echo "checked";?> data-c_id="{{ $product->id }}" class="feed">
                                <span class="slider round"></span>
                            </label>
                        </td>
                        <td>
                            <label class="switch">
                                <input onchange="facebookFeed(this)"  value="{{ $product->id }}" type="checkbox" <?php if($product->facebook == 'Yes' && $product->current_stock > 0) echo "checked";?> data-c_id="{{ $product->id }}" class="feed">
                                <span class="slider round"></span>
                            </label>
                        </td>
                        <td>
...
@section('script')
    <script type="text/javascript">
        ...
        function update_published(el){
            if(el.checked){
                var status = 1;
                var is_blocked_by_admin = 0;
            }
            else{
                var status = 0;
                var is_blocked_by_admin = 1;
            }
            $.post('{{ route('products.published') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status,is_blocked_by_admin:is_blocked_by_admin}, function(data){
                if(data == 1){
        ...
        function googleFeed(el){
            ...
            $.post('{{ route('products.googlefeed') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
        ...
        function facebookFeed(el){
            ...
            $.post('{{ route('products.facebookfeed') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    showAlert('success', 'Facebook Feed updated successfully');
                }
                else{
                    showAlert('danger', 'Something went wrong');
                }
